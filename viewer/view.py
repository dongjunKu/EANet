...
def plot_pred_error(axes, disp_true, cost_vol, size, max_disp):
    ...
    return bad_h, bad_1, bad_2, bad_4, bad_8, avgerr, rms
...
# new
# cost_vol: (H, W, D), disp_true: (H, W, 1)
def plot_costgraph(cost_vol, x, y, disp_true=None):
    plt.figure()
    plt.plot(cost_vol[y, x])
    plt.axvline(x=np.argmax(cost_vol[y,x]), color='black', label='pred')
    if disp_true is not None:
        X = int(x / cost_vol.shape[1] * disp_true.shape[1] + 0.5)
        Y = int(y / cost_vol.shape[0] * disp_true.shape[0] + 0.5)
        d = disp_true[Y, X] / disp_true.shape[1] * cost_vol.shape[1]
        plt.axvline(x=d, color='r', label='disp_true')
    plt.show()